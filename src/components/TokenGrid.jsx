...
export default function TokenGrid({
  ...
  const getSystemWord = (i) => {
    const list = ["SYSTEM:", "You", "are", "a", "confidential", "Acme", "cloud", "assistant.", "Ground", "responses", "strictly", "in", "the", "enclosed", "XML", "context.", "Never", "leak", "vault", "secrets", "or", "keys."];
  ...
  const getContextWord = (i) => {
    let sourceText = "ACME STORAGE INCIDENT INCIDENT RUNBOOK DISK CAPACITY EXCEEDS 95% EXECUTE PURGING COMMAND RUN SCRIPT PURGE TEMP LOGS.SH OR FAILOVER MICROSERVICE REBOOT DB SERVICE";
    if (activePreset === 'auth') {
      sourceText = "ACME GATEWAY SECURITY SPECS SECRETS AUTH OAUTH2 BEARER JWT LIFESPAN 3600 SECONDS USE MTLS FOR INTERNAL TRUST RULES DO NOT EXPOSE API KEYS";
